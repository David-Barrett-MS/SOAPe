    ...
    public class ClassLogger: IDisposable
    {
        ...
        public void LoadLogFolder(string LogFolder)
        {
            try
            {
                ...
                foreach (string file in files)
                {
                    ...
                    OnProgressChanged(new ProgressEventArgs(String.Format("Processing file {0} of {1}", i++, files.Length), percentComplete));
        ...
        public void LoadLogFile(string LogFileName, bool ClearHistory = true, bool ShowProgress = true)
        {
            ...
            try
            {
                ...
                if (!bFoundTraceTag)
                {
                    // No <trace> tags found, so check if we just have single Xml dump
                    String sTrace = String.Empty;
                    readLogFileStream = new FileStream(LogFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                    using (StreamReader logFileReader = new StreamReader(readLogFileStream))
                    {
                        sTrace = logFileReader.ReadToEnd().Trim();
                    }
                    
                    if (sTrace.StartsWith("<?xml"))
                    {
                        ParseGenericRequest(sTrace);
                    }
                    else if (!String.IsNullOrEmpty(sTrace))
                    {
                        // Doesn't appear to be an Xml file, but we'll do a final check by adding the Xml descriptor
                        // to the file to see if that works
                        ParseGenericRequest(String.Concat("<?xml version=\"1.0\" encoding=\"utf-8\"?>", sTrace));
                    }
                }
            }
            catch (Exception ex)
        ...
        private void ParseTrace(string Trace)
        {
            ...
            try
            {
                ...
                try
                {
                    threadId = int.Parse(xml.FirstChild.Attributes["Tid"].Value);
                }
                catch { }
        ...
        public void ParseGenericRequest(string Request)
        {
            ...
            try
            {
                ...
                {
                    if (sEWSMethod.ToLower().Contains("response"))
                    {
                        sTag = "EwsResponse";
                    }
                }
