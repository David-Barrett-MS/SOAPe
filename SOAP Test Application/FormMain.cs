...
namespace SOAPe
{
    public partial class FormMain : Form
    {
        ...
        private void UpdateTextBoxState()
        {
            ...
            checkBoxSavePassword.Visible = bUserCredsVisible;
        ...
        {
            // Save our configuration (URL, username, domain) to the registry

            Microsoft.Win32.RegistryKey oKey = null;
            try
            {
                string sRootKey = "Software\\" + System.Windows.Forms.Application.CompanyName + "\\" + System.Windows.Forms.Application.ProductName;
                oKey = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(sRootKey, true);
                if (oKey == null)
                    oKey = Microsoft.Win32.Registry.CurrentUser.CreateSubKey(sRootKey);

                if (radioButtonUrlCustom.Checked)
                    radioButtonUrlCustom.Tag = textBoxURL.Text;
                oKey.SetValue("EWSUrl", (string)radioButtonUrlCustom.Tag);
                oKey.SetValue("CustomUrl", radioButtonUrlCustom.Checked);

                if (radioButtonCertificateAuthentication.Checked)
                {
                    oKey.SetValue("AuthenticationMethod", "Certificate");
                }
                else if (radioButtonSpecificCredentials.Checked)
                {
                    oKey.SetValue("AuthenticationMethod", "Credentials");
                }
                else if (radioButtonOAuth.Checked)
                {
                    oKey.SetValue("AuthenticationMethod", "OAuth");
                }
                else if (radioButtonNoAuth.Checked)
                {
                    oKey.SetValue("AuthenticationMethod", "None");
                }
                else
                    oKey.SetValue("AuthenticationMethod", "Default");

                oKey.SetValue("Username", textBoxUsername.Text);
                oKey.SetValue("Domain", textBoxDomain.Text);
                oKey.SetValue("SavePassword", checkBoxSavePassword.Checked);
                if (checkBoxSavePassword.Checked)
                {
                    SecureString ssPassword = new SecureString();
                    for (int i = 0; i < textBoxPassword.Text.Length; i++)
                        ssPassword.AppendChar(textBoxPassword.Text[i]);
                    //oKey.SetValue("Password", ssPassword.ToString());
                    oKey.SetValue("Password", textBoxPassword.Text);
                }
                else
                    oKey.DeleteValue("Password", false);

                oKey.SetValue("AuthCertificate", textBoxAuthCertificate.Text);

                oKey.SetValue("IgnoreSSLCertificateErrors", checkBoxIgnoreCertErrors.Checked);
                oKey.SetValue("BypassIEProxy", checkBoxBypassProxySettings.Checked);

                oKey.SetValue("UpdateEWSHeader", checkBoxUpdateEWSHeader.Checked);
                oKey.SetValue("ExchangeVersion", comboBoxRequestServerVersion.Text);
            }
            catch { }
        ...
        private void LoadSettings()
        {
            ...
            {
                string sRootKey = "Software\\" + System.Windows.Forms.Application.CompanyName + "\\" + System.Windows.Forms.Application.ProductName;
                oKey = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(sRootKey, false);

                radioButtonUrlCustom.Tag = (string)oKey.GetValue("EWSUrl");
                radioButtonUrlOffice365.Checked = !((oKey.GetValue("CustomUrl") as string).ToLower() == "true");
                if (!radioButtonUrlOffice365.Checked)
                    radioButtonUrlCustom.Checked = true;

                textBoxUsername.Text = (string)oKey.GetValue("Username");
                textBoxDomain.Text = (string)oKey.GetValue("Domain");

                // Read authentication method
                string authMethod = (string)oKey.GetValue("AuthenticationMethod");
                if (!String.IsNullOrEmpty(authMethod))
                {
                    // This key was added in 1.0.9, previous registry keys will be read but are no longer written
                    switch (authMethod.ToLower())
                    {
                        case "certificate":
                            radioButtonCertificateAuthentication.Checked = true;
                            break;
                        ...
                        case "oauth":
                            radioButtonOAuth.Checked = true;
                            break;

                        case "none":
                            radioButtonNoAuth.Checked = true;
                            break;

                        default:
                            radioButtonDefaultCredentials.Checked = true;
                            break;
                    }
                }
                else
                {
                    // This is the logic prior to 1.0.9, kept so as to not lose settings on upgrade
                    bool bUseDefault = ((oKey.GetValue("UseDefaultCredentials") as string).ToLower() == "true");
                    if (bUseDefault)
                    {
                        radioButtonDefaultCredentials.Checked = true;
                    }
                    ...
                    {
                        bool bUseOAuth = ((oKey.GetValue("UseOAuth") as string).ToLower() == "true");
                        if (bUseOAuth)
                        {
                            radioButtonOAuth.Checked = true;
                        }
                ...
                string sAuthCertificate = (string)oKey.GetValue("AuthCertificate");
                LoadCertificate(sAuthCertificate);

                bool bSavePassword = ((oKey.GetValue("SavePassword") as string).ToLower() == "true");
                _suppressPasswordWarning = true;
                checkBoxSavePassword.Checked = bSavePassword;
                if (bSavePassword)
                    textBoxPassword.Text = (string)oKey.GetValue("Password");
                _suppressPasswordWarning = false;

                checkBoxIgnoreCertErrors.Checked = ((oKey.GetValue("IgnoreSSLCertificateErrors") as string).ToLower() == "true");
                checkBoxBypassProxySettings.Checked = ((oKey.GetValue("BypassIEProxy") as string).ToLower() == "true");

                comboBoxRequestServerVersion.Text = (oKey.GetValue("ExchangeVersion") as string);
                if (String.IsNullOrEmpty(comboBoxRequestServerVersion.Text))
                    comboBoxRequestServerVersion.Text = "Exchange2010_SP1";
                checkBoxUpdateEWSHeader.Checked = ((oKey.GetValue("UpdateEWSHeader") as string).ToLower() == "true");
            }
            catch { }
            ...
            _logger.DebugLog("Settings loaded");
        }
        ...
        {
            if ((!checkBoxSavePassword.Checked) || _suppressPasswordWarning)
                return;

            if (System.Windows.Forms.MessageBox.Show("Enabling this option will store the password insecurely in the registry.\r\n\r\nAre you sure you wish to do this?",
                "Confirm save password", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.No)
